...
def test_wake_list_changes_are_explicit():
    ...
    assert len(WAKE_PATTERNS) == 183, (
        ...
        f"batch-only -2 → 142; D0 read-only dynamic continuation "
        f"proposal +1:workflow.fragment.proposed → 143; aggregate rebuild "
        f"redrive +1:fanout.aggregate.rebuild.requested → 144; restart-safe "
        f"mechanical completion reconciliation +1:"
            f"run.manager.tick.completed → 145; reader fanout async spawn "
            f"liveness +1:worker.launch_artifact.written → 146; channel "
            f"synthesis request bridge +1:channel.synthesis.requested → 147; "
            f"Codex provider-native turn close bridge +1:"
            ...
            f"self-evolution resident settlement +5:autoresearch.loop.completed / "
            f"evolution.trial.execution.completed / "
            f"evolution.trial.execution.failed / evolution.canary.completed / "
            f"evolution.canary.failed → 178; evolution environment preflight "
            f"failure +1 → 179; Skill optimizer selection/execution terminals "
            f"+3 → 182; Channel adaptive next-round proposal +1 → 183"
        )
